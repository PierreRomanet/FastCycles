...
PROGRAM main
  ...
  !
  CALL h5open_f(hdferr) !  hdferr: error
  !
  ! Initialize data.
  !
  DO i = 1, dim0
  ...
  !
  CALL h5fcreate_f(filename, H5F_ACC_TRUNC_F, file, hdferr) ! Name of the file, H5F_ACC_TRUNC: overwrite previous file, file_id (out), error
  !
  ! Create dataspace.  Setting maximum size to NULL sets the maximum
  ! size to be the current size.
  !
  CALL h5screate_simple_f(2, dims, space, hdferr) ! Number of dimension, size of dimension (array), space id (out), error
  !
  ! Create the dataset creation property list, set the layout to
  ! compact.
  !
  CALL h5pcreate_f(H5P_DATASET_CREATE_F, dcpl, hdferr) ! Type of property list, property id, error 
  CALL h5pset_layout_f(dcpl, H5D_COMPACT_F, hdferr) ! Set the type of storage (H5D_CONTIGUOUS or CHUNKED)
  !
  ! Create the dataset.  We will use all default properties for this
  ! example.
  !
  CALL h5dcreate_f(file, dataset, H5T_STD_I32LE, space, dset, hdferr, dcpl) ! File name, name of dataset, datatype, dataspace, data set (out), error, property
  ! 
  ! Write the data to the dataset.
  !
  CALL h5dwrite_f(dset, H5T_NATIVE_REAL, wdata, dims, hdferr)
  !
